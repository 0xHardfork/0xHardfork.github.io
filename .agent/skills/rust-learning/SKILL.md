...
[EXTREMELY exhaustive, double-length explanation. Start from basics but dive incredibly deep into memory layout and compiler mechanics.]
...
Generate exactly 8 exercises, pacing from easy to hard, to thoroughly drill today's concepts.
...
- **Tone & Style**: Use a warm, human-like, and conversational tone. Write like an experienced mentor chatting with a colleague. Use simple, colloquial language, and absolutely avoid robotic "AI-speak" (e.g., "In this tutorial we will explore...", "Welcome to day X..."). Let the writing flow naturally without feeling like a rigid template.
- **Exhaustive Content Length**: You MUST generate extremely long, exhaustive content to ensure absolutely NO knowledge points or details are missed. Dive deeply into every nuance, memory implication, and compiler behavior. Do NOT summarize or use brief overviews. Your output must be a definitive, deeply comprehensive guide.
- **Target Audience & Balance**: Cater to developers with strong experience in other languages but who are **completely new to Rust**. You MUST balance the content: start by thoroughly explaining the foundational syntax and concepts so beginners aren't lost, and then seamlessly scale up to advanced mechanics and under-the-hood performance details for the experienced engineer.
- **Cross-Language Analogies**: Frequently compare Rust's approach to Garbage Collection (Java/Go/Python) or Manual Memory Management (C/C++). For example, relate Rust's `Result` to Go's `err` returns or Java's Exceptions.

### Concept Explanations
- Break down the syntax and semantics meticulously.
- Be clear and precise about **Rust semantics**, ownership, borrowing, and lifetimes.
- Highlight common pain points (like "fighting the borrow checker") and how to systematically overcome them.
- Use professional terminology, but always define Rust-specific terms (like "turbofish", "interior mutability", "slice") when first introduced.

### Code Examples
- **Volume & Variety**: Provide MULTIPLE code examples. Show basic usage, advanced idiomatic usage, and edge cases. Never provide just one simple snippet.
...
### Exercises
- **Volume**: You MUST generate exactly **8 practice exercises** per daily session.
- Progressive difficulty from very basic syntax to extremely tricky ownership/borrowing scenarios.
